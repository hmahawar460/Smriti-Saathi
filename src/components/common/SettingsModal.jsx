    ...
    <div className="fixed inset-0 z-[10060] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-left">
          {/* User Profile / Account Status Section */}
          {currentUser ? (
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3.5 min-w-0">
                <div className="w-13 h-13 rounded-2xl bg-gradient-to-tr from-[#001F54] to-[#0D7377] text-white flex items-center justify-center text-xl font-black shadow-md border-2 border-white shrink-0">
                  {currentUser.name ? currentUser.name.charAt(0).toUpperCase() : "U"}
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h4 className="font-black text-base text-[#001F54] leading-tight truncate">
                      {currentUser.name}
                    </h4>
                  ...
                  <p className="text-xs text-slate-500 font-medium mt-0.5 truncate">
                    {isDoctor
                      ? (currentUser.hospital || currentUser.specialty || "Cognitive Neurologist")
                      : <>ID: <span className="font-mono font-bold text-slate-700">{currentUser.patientCode || profile?.patientCode || "PT-7241"}</span> · {currentUser.age || profile?.age || "70"} Yrs</>
                    }
                  </p>
                </div>
              </div>
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-r from-blue-50/70 via-slate-50 to-teal-50/40 border border-blue-200/80 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3.5 min-w-0">
                <div className="w-13 h-13 rounded-2xl bg-slate-200 text-slate-600 flex items-center justify-center text-xl font-black shadow-2xs border-2 border-white shrink-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider bg-slate-200 text-slate-700 border border-slate-300">
                      Not Signed In
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 font-medium mt-0.5 truncate">
                    Sign in to sync scores, clinical tests & connect with your doctor
                  </p>
                </div>
              </div>
              ...
              <button
                type="button"
                id="platform-settings-guest-signin-btn"
                onClick={() => {
                  onClose();
                  if (onOpenAuthModal) onOpenAuthModal({ mode: "login" });
                }}
                className="px-4 py-2 rounded-xl bg-[#2563eb] hover:bg-[#1d4ed8] text-white text-xs font-bold transition cursor-pointer shadow-xs shrink-0 flex items-center gap-1.5"
              >
          ...
          <div className="pt-2 border-t border-slate-200 flex items-center justify-between gap-3">
            ...
            {currentUser ? (
              <button
                type="button"
                id="platform-settings-logout-btn"
                onClick={() => {
                  onClose();
              ...
              >
                ...
                <span>Sign Out ({currentUser.name?.split(" ")[0] || "Account"})</span>
              </button>
            ) : (
              <button
                type="button"
                id="platform-settings-login-footer-btn"
                onClick={() => {
                  onClose();
                  if (onOpenAuthModal) onOpenAuthModal({ mode: "login" });
                }}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-blue-50 hover:bg-blue-100 border border-blue-200 text-blue-700 font-black text-xs transition cursor-pointer active:scale-95 shadow-2xs"
              >
