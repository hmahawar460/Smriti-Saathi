...
export const FloatingVoiceButton = ({
  ...
  return (
    ...
    >
      {/* Floating Helper Tooltip (Desktop/Tablet only to avoid mobile clutter) */}
      {(showTooltip || isHovered) && !isOpen && (
        <div
          className="pointer-events-auto mb-2.5 mr-1 px-3.5 py-1.5 rounded-xl bg-slate-900/90 backdrop-blur-md text-white shadow-xl border border-slate-700/60 text-xs font-semibold hidden sm:flex items-center gap-2 animate-in fade-in slide-in-from-bottom-2 duration-200"
          role="tooltip"
        >
      ...
      {/* Main Floating Action Button: Clean round circle on mobile, pill with label on desktop */}
      <button
        ...
        className={`pointer-events-auto group relative flex items-center justify-center sm:justify-start gap-2.5 sm:gap-3 w-12 h-12 aspect-square sm:w-auto sm:h-auto sm:aspect-auto p-0 sm:px-4 sm:py-3.5 rounded-full text-white shadow-2xl transition-all duration-300 cursor-pointer active:scale-95 shrink-0 ${
          isOpen
      ...
      >
        ...
        <div className="relative z-10 w-full h-full sm:w-10 sm:h-10 rounded-full sm:bg-white/15 sm:border sm:border-white/25 flex items-center justify-center text-white shrink-0 group-hover:scale-105 transition-transform sm:shadow-inner">
          <Mic className={`w-5 h-5 sm:w-5.5 sm:h-5.5 ${isOpen ? "text-[#9DF3C4] animate-bounce" : "text-white"}`} />

          {/* Active Live Audio Status Beacon */}
          <span className="absolute top-1 right-1 sm:-top-0.5 sm:-right-0.5 flex h-2.5 w-2.5 sm:h-3 sm:w-3">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2.5 w-2.5 sm:h-3 sm:w-3 bg-emerald-500 border border-white"></span>
          </span>
        </div>

        {/* Audio Equalizer Animated Waves (Subtle Voice Indicator - Desktop/Tablet only) */}
        ...
        {/* Label & AI Badge (Desktop/Tablet only) */}
