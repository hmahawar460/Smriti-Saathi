...
export const PhysicalActivityEngine = ({
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-9 shadow-xl border border-emerald-200 text-center space-y-5">
          ...
          <div>
            ...
            <h2 className="text-3xl font-extrabold text-[#132A2F] font-display mt-2">
              Great Job, {profile.preferredName}!
            </h2>
      ...
      /* Stats Badges */
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div className="bg-[#F3F8F7] p-3.5 rounded-2xl border border-[#0D7377]/15">
              <span className="text-[11px] font-bold text-slate-500 block uppercase">Duration</span>
              <span className="text-xl font-black text-[#0D7377]">
                {Math.max(1, Math.ceil(elapsedSeconds / 60))} min
              </span>
            </div>

            {activity.repetitions ? <div className="bg-[#F3F8F7] p-3.5 rounded-2xl border border-[#0D7377]/15">
                <span className="text-[11px] font-bold text-slate-500 block uppercase">Reps Done</span>
                <span className="text-xl font-black text-[#132A2F]">
                  {repsCount || activity.repetitions} Reps
                </span>
              </div> : <div className="bg-[#F3F8F7] p-3.5 rounded-2xl border border-[#0D7377]/15">
                <span className="text-[11px] font-bold text-slate-500 block uppercase">Pace</span>
                <span className="text-xl font-black text-[#132A2F]">Comfortable</span>
              </div>}

            <div className="bg-[#F3F8F7] p-3.5 rounded-2xl border border-[#0D7377]/15 col-span-2 sm:col-span-1">
              <span className="text-[11px] font-bold text-slate-500 block uppercase">Status</span>
              <span className="text-lg font-black text-emerald-700">Saved Locally ✓</span>
            </div>
          </div>
  ...
  return <div className="max-w-2xl mx-auto px-4 py-4 sm:py-6 space-y-4 animate-in fade-in duration-200">
  ...
  >
    ...
    /* Breathing visualizer for Breathing activity */
          ...
          {activity.id === "phys-4" ? <div className="flex flex-col items-center space-y-3">
              <div
    className={`w-32 h-32 rounded-full border-8 border-teal-400 bg-teal-100 flex items-center justify-center text-[#0D7377] transition-all duration-3000 ${isActive && breathPhase === "Inhale" ? "scale-125 bg-teal-200" : isActive && breathPhase === "Hold" ? "scale-115 bg-teal-100 ring-8 ring-teal-200" : "scale-90 bg-teal-50"}`}
  >
            ...
            </div> : <div className="flex flex-col items-center space-y-3">
    ...
    /* Reps counter if applicable */
          ...
          {activity.repetitions && <div className="mt-4 flex items-center gap-3 bg-white px-4 py-2 rounded-2xl border border-slate-200 shadow-xs">
  ...
  >
    ...
    className={`px-8 py-3.5 rounded-2xl font-black text-base flex items-center gap-2 shadow-md transition active:scale-95 cursor-pointer ${isActive ? "bg-amber-500 hover:bg-amber-600 text-white" : "bg-[#0D7377] hover:bg-[#0A5C5F] text-white"}`}
  >
    ...
    /* Step-by-Step Instructions */
        ...
        <div className="text-left bg-slate-50 rounded-2xl p-4 sm:p-5 border border-slate-200 space-y-3">
          ...
          <ol className="space-y-2.5">
            {activity.instructions.map((step, idx) => <li
    key={idx}
    onClick={() => {
      setCurrentStepIndex(idx);
      speakInstruction(`Step ${idx + 1}. ${step}`);
    }}
    className={`p-3 rounded-xl border transition cursor-pointer flex items-start gap-2.5 ${currentStepIndex === idx ? "bg-teal-50 border-[#0D7377] shadow-xs text-[#132A2F]" : "bg-white border-slate-200 text-slate-700 hover:border-teal-200"}`}
  >
